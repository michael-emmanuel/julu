    ...
    <nav className="relative">
      ...
      space-x-10 sm:space-x-20 overflow-x-scroll scrollbar-hide">
        {Object.entries(requests).map(([key, { title, url }]) => (
          <h2 
            key={key} 
            onClick={() => router.push(`/?genre=${key}`)} // Take advantage of NextJS built in router
      ...
      <div className="absolute top-0 right-0 bg-gradient-to-l from-[#06202A] h-10 w-1/12" /> {/* dont give a to color to make transparent */}
    </nav>
  )
}
